document.addEventListener("DOMContentLoaded", () => {
    ...
    function triggerProcess() {
        ...
        stompClient.send("/app/vigenere", {}, JSON.stringify(payload));
    }

    textoInput.addEventListener('input', triggerProcess);
    claveInput.addEventListener('input', triggerProcess);
    idiomaSelect.addEventListener('change', triggerProcess);

    btnCifrar.addEventListener('click', () => {
        operacionActual = "CIFRAR";
        btnCifrar.classList.replace('bg-slate-700', 'bg-emerald-600');
        btnCifrar.classList.replace('text-slate-300', 'text-white');
        btnDescifrar.classList.replace('bg-emerald-600', 'bg-slate-700');
    ...
    btnDescifrar.addEventListener('click', () => {
        ...
        btnDescifrar.classList.replace('bg-slate-700', 'bg-emerald-600');
        btnDescifrar.classList.replace('text-slate-300', 'text-white');
        btnCifrar.classList.replace('bg-emerald-600', 'bg-slate-700');
